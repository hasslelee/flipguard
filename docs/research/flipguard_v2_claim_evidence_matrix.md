...
| Automatic candidate generation | Direct digest-bound synthesis of LogN/LogQ/LogP/scale; backend feasibility raises non-generatable NTT-prime sizes; both seed-0 policy matrices selected 10/10 workloads | IMPLEMENTED_NOT_EVALUATED | Current scope is three tabular forms, scalar-replicated packing, and rescale-aware Lattigo v6; one split and one key per workload | Predeclared multi-seed direct-synthesis study, repeated keys, locked audit, frozen-oracle comparison, and external providers | “Fully general automatic tuner” |
...
| Adaptive encrypted-trial reduction | Conservative seed-0 matrix used 10 trials; low-floor ablation used 14 trials, including four observed REJECTED-to-SAFE scale repairs, versus 220 in the fixed-catalog protocol | IMPLEMENTED_NOT_EVALUATED | One split, one key per workload, no locked audit, and the 93.64-95.45% reduction is a protocol-count comparison rather than a global-oracle result | Multi-seed trial-count distribution, repeated keys, false NO_SAFE analysis, locked audit, and bounded-oracle regret | “One encrypted trial always suffices” |
...
| Robustness across cryptographic randomness | Contract schema 2 and executor aggregate sample scores across fresh keypairs; three-key smoke and banknote linear repair development runs completed | IMPLEMENTED_NOT_EVALUATED | Only one primary workload has a three-key adaptive result and encryption randomness is not separately seeded | Full multi-workload key-repeat distribution and separately locked audit keys | “Key-independent safety” |
