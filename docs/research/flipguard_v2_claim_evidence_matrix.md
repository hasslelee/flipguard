...
|---|---|---:|---|---|
| Direct synthesis | SUPPORTED | false | Final clean-source selection covers 50 primary instances; separate BSDS500 Sobel and Harris adapters also synthesize and select without catalog lookup | Paper admission remains pending; scope is the declared adapters only |
...
| Locked audit on fixed held-out partitions | SUPPORTED | false | Primary final audits pass without retuning; deeper `mlp_square_poly3` preserves 24/25 PASS plus one disclosed REJECT; BSDS500 Sobel and Harris pass over disjoint image clusters | Support is limited to the declared repeated partitions and single-window image scopes |
| Structural generalization | PARTIALLY_SUPPORTED | false | Full `mlp_square_poly3` selection is 25/25 with audit 24 PASS and 1 numerical REJECT; BSDS500 Sobel and depth-2 Harris each pass selection/audit over 50+50 disjoint image clusters | Arbitrary graphs, packed full-image operators, and CNN/LeNet remain unevaluated |
...
| Artifact reproducibility | PARTIALLY_SUPPORTED | false | Final confirmatory, Sobel, and Harris packs have deterministic rebuild verifiers, raw ledgers, policy/binary/input digests, and pushed source commits | Release tag, external archive, and independent-machine replay remain |
