...
| Automatic candidate generation | Direct digest-bound synthesis of LogN/LogQ/LogP/scale plus one-trial SAFE development runs for banknote linear and MLP | IMPLEMENTED_NOT_EVALUATED | Current scope is three tabular forms, scalar-replicated packing, and rescale-aware Lattigo v6 | Predeclared multi-model/multi-seed direct-synthesis study against the frozen oracle and external providers | “Fully general automatic tuner” |
...
| Adaptive encrypted-trial reduction | Banknote MLP and linear development runs selected the initial direct candidate in one encrypted trial | IMPLEMENTED_NOT_EVALUATED | Two workloads, one split, one key, and no forced-repair study | Multi-seed trial-count distribution, repair-trigger cases, false NO_SAFE, and oracle regret | “One encrypted trial always suffices” |
