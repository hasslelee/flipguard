...
| Automatic candidate generation | Direct digest-bound synthesis of LogN/LogQ/LogP/scale; backend feasibility raises non-generatable NTT-prime sizes; seed-0 three-key matrix selected 10/10 workloads | IMPLEMENTED_NOT_EVALUATED | Current scope is three tabular forms, scalar-replicated packing, rescale-aware Lattigo v6, and one data split | Predeclared multi-split direct-synthesis study, locked audit, frozen-oracle comparison, and external providers | “Fully general automatic tuner” |
...
| Adaptive encrypted-trial reduction | Seed-0 three-key low-floor matrix used 14 configuration trials/42 key runs, including four REJECTED-to-SAFE scale repairs, versus 220 configurations/660 key runs for an equally repeated fixed catalog | PARTIALLY_SUPPORTED | One split, no locked audit, and the 93.64% reduction is a bounded-protocol count comparison rather than a global-oracle result | Multi-split trial distribution, false NO_SAFE analysis, locked audit, and bounded-oracle regret | “One encrypted trial always suffices” |
...
| Robustness across cryptographic randomness | Contract schema 2 aggregates sample scores across fresh keypairs; seed-0 matrix completed 42 key runs and all 10 final certificates had zero flips/violations | PARTIALLY_SUPPORTED | Three keys on one validation split are too few for a key-independence claim; encryption randomness is not separately seeded | More key repeats across multiple splits and separately locked audit keys | “Key-independent safety” |
