...
| Automatic candidate generation | Direct digest-bound synthesis of LogN/LogQ/LogP/scale; seed-0 development matrix selected 10/10 workloads with zero flips/violations on Vcert=1610 | IMPLEMENTED_NOT_EVALUATED | Current scope is three tabular forms, scalar-replicated packing, and rescale-aware Lattigo v6; a 30-bit scale floor hid dataset-specific variation | Predeclared multi-seed direct-synthesis study, lower-floor stress protocol, frozen-oracle comparison, and external providers | “Fully general automatic tuner” |
...
| Adaptive encrypted-trial reduction | Seed-0 development matrix used one encrypted configuration trial for each of 10 workloads: 10 total versus 220 in the fixed-catalog protocol | IMPLEMENTED_NOT_EVALUATED | One split, one execution per workload, no forced-repair study, and the 95.45% reduction is a protocol-count comparison rather than a global-oracle result | Multi-seed trial-count distribution, repeated keys, repair-trigger cases, false NO_SAFE, and bounded-oracle regret | “One encrypted trial always suffices” |
