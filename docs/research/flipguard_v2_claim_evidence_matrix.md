...
| Provider-neutral candidate admission | PARTIALLY_SUPPORTED | false | Manual, bounded-catalog, direct, and synthetic external-format arms share one verified gate; exact Orion public configurations fail closed before execution; one source-replayed AWS HIT literal imports losslessly and passes Security V2 but is decision-REJECTED. A no-rerun matched-workload audit binds the same raw source, model, graph, threshold, alpha, and margin floor: direct is SAFE and passes locked audit, while HIT has six validation violations and is not audited. A pinned EVA v1.0.1 compiler replay exports an exact Security-V2-admitted Q/P literal and bound schedule; its corrected Lattigo smoke is decision-REJECTED. The same compiler output executes 42/42 observations on native SEAL but is also validation-REJECTED with 11 flips and 36 budget violations, so locked audit is not run | Native execution is supported, but no native third-party compiler or selector has produced a SAFE candidate that completed no-retuning locked audit; the HIT timing values are unpaired diagnostics; HIT is a formula selector, EVA is a compiler, and neither result supports general search-autotuner integration or cross-runtime equivalence |
...
| Artifact reproducibility | PARTIALLY_SUPPORTED | false | Final confirmatory, Sobel, Harris, MNIST CNN-lite, independent-training-seed, direct-ablation, conditional-analytical-readiness, exact-security-estimator, HIT, and EVA packs have deterministic rebuild or fail-closed ledger verifiers, raw ledgers or static inventories, policy/binary/input digests where applicable, and pushed source commits | Nine overwritten independent-seed pre-CKKS recovery logs are disclosed; release tag and external archive remain |
