...
func WriteCKKSMulProbeMarkdown(path string, results []ckksbackend.MultiplicationProbeResult) error {
	...
	fmt.Fprintf(f, "This report summarizes ciphertext-ciphertext multiplication probe results in the CKKS backend.\n\n")
	fmt.Fprintf(f, "The probe evaluates:\n\n")
	fmt.Fprintf(f, "```text\n")
	fmt.Fprintf(f, "z2 = z * z\n")
	fmt.Fprintf(f, "```\n\n")
	fmt.Fprintf(f, "Compared methods:\n\n")
	fmt.Fprintf(f, "- `mul_only`\n")
	fmt.Fprintf(f, "- `mul_relin`\n")
	fmt.Fprintf(f, "- `mul_rescale`\n")
	fmt.Fprintf(f, "- `mul_relin_rescale`\n\n")
	fmt.Fprintf(f, "The goal is to compare output degree, level consumption, and decoded error before implementing the full encrypted polynomial path.\n\n")
	...
	fmt.Fprintf(f, "\n## Results\n\n")
	fmt.Fprintf(f, "| Index | Method | z | Plain z2 | Selected z2 | Mode | Error | In Degree | Out Degree | Initial Level | Final Level |\n")
	fmt.Fprintf(f, "|---:|---|---:|---:|---:|---|---:|---:|---:|---:|---:|\n")

	for i, result := range results {
		fmt.Fprintf(
			f,
			"| %d | %s | %.6f | %.10f | %.10f | %s | %.10f | %d | %d | %d | %d |\n",
			i,
			result.Method,
			result.Z,
			result.PlainZ2,
