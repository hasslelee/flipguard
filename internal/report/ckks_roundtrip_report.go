...
func WriteCKKSRoundTripMarkdown(path string, result ckksbackend.RoundTripResult) error {
	...
	fmt.Fprintf(f, "\n---\n\n")
	fmt.Fprintf(f, "## 한국어\n\n")
	fmt.Fprintf(f, "이 문서는 최소 Lattigo CKKS encode-encrypt-decrypt-decode roundtrip 결과 요약.\n\n")
	fmt.Fprintf(f, "이 probe는 CKKS parameter 생성, key generation, 실수 값 encoding, encryption, decryption, decoding, 원본 값과 decoded 값 비교가 현재 repository에서 동작하는지 확인한다.\n\n")
	fmt.Fprintf(f, "아직 완전한 FlipGuard encrypted inference 실험은 아니다. Encrypted `logreg_small` evaluation 구현 전 backend 사용 가능성 점검 단계다.\n\n")
