...
const BSDS500HarrisExecutionAdapterV1 = "bsds500_harris_square_rescale_graph_adapter_v1"

type HarrisSampleObservation = SobelSampleObservation

type HarrisTrialResult = SobelTrialResult

type AdaptiveHarrisAutotuneResult struct {
	...
	Trials     []HarrisTrialResult `json:"trials"`
...
func ExecuteHarrisCandidate(
	contract WorkloadContract,
	candidate SynthesizedCandidate,
	trialIndex int,
) (HarrisTrialResult, error) {
	if err := contract.Validate(); err != nil {
		return HarrisTrialResult{}, fmt.Errorf(
			"validate Harris execution contract: %w",
			err,
		)
	}
	if contract.ModelType != BSDS500HarrisModelType {
		return HarrisTrialResult{}, fmt.Errorf(
			"unsupported Harris contract model_type %q",
			contract.ModelType,
		)
	}
	if trialIndex <= 0 {
	...
	if err := verifyContractArtifacts(contract); err != nil {
		return HarrisTrialResult{}, err
	}
	result := HarrisTrialResult{
		TrialIndex:          trialIndex,
		Candidate:           candidate,
		KeyRepeatsRequested: contract.Deployment.ValidationKeyRepeats,
		SampleLedger: make(
			[]HarrisSampleObservation,
			0,
	...
	if candidate.Path != "rescale" {
		return HarrisTrialResult{}, fmt.Errorf(
			"Harris adapter requires rescale-aware candidate",
		)
	}
	...
	for keyRun := 1; keyRun <= contract.Deployment.ValidationKeyRepeats; keyRun++ {
		...
		records, err := context.RunCKKSHarrisPatchInference(
			ckksbackend.CKKSHarrisPatchConfig{
		...
		if err != nil {
			result.Status = certify.StatusFailed
			result.Assurance = certify.AssuranceNone
			result.Reason = "encrypted Harris execution failed"
			result.Failure = err.Error()
			result.FailureSignal = classifyExecutionFailure(err)
			return result, nil
		}
		if keyRun > 1 && len(records) != len(sampleOrder) {
			return HarrisTrialResult{}, fmt.Errorf(
				"Harris sample count changed on key run %d",
				keyRun,
			)
		}
		for index, record := range records {
			...
			if keyRun == 1 {
				...
				return HarrisTrialResult{}, fmt.Errorf(
					"Harris sample identity changed on key run %d at index %d",
					keyRun,
					index,
				)
			}
	...
	if digestValidationScores(sampleOrder, plainScores) !=
		...
		return HarrisTrialResult{}, fmt.Errorf(
			"Harris backend validation scope digest mismatch",
		)
	}
	...
	if err != nil {
		return HarrisTrialResult{}, fmt.Errorf(
			"aggregate Harris encrypted validation: %w",
			err,
		)
	}
	if aggregation.Coverage.VCert !=
		...
		return HarrisTrialResult{}, fmt.Errorf(
			"Harris certification partition does not match contract",
		)
	}
	...
	if result.SuccessRuns !=
		contract.Deployment.ValidationKeyRepeats {
		return HarrisTrialResult{}, fmt.Errorf(
			"Harris certificate key run count mismatch",
		)
	}
...
func RunAdaptiveHarrisAutotune(
	plan SynthesisPlan,
) (AdaptiveHarrisAutotuneResult, error) {
	if plan.SchemaVersion != SynthesisPlanSchemaVersion ||
		len(plan.InitialCandidates) != 1 {
		return AdaptiveHarrisAutotuneResult{}, fmt.Errorf(
			"invalid Harris synthesis plan",
		)
	}
	recomputedDigest, err := digestContract(plan.Contract)
	if err != nil {
		return AdaptiveHarrisAutotuneResult{}, err
	}
	if recomputedDigest != plan.ContractDigest {
		return AdaptiveHarrisAutotuneResult{}, fmt.Errorf(
			"Harris synthesis plan contract digest mismatch",
		)
	}
	result := AdaptiveHarrisAutotuneResult{
		SchemaVersion:    SynthesisPlanSchemaVersion,
		ExecutionAdapter: BSDS500HarrisExecutionAdapterV1,
		Plan:             plan,
		Trials:           make([]HarrisTrialResult, 0),
	}
	...
	for trialIndex := 1; trialIndex <= maxTrials; trialIndex++ {
		...
		if trial.Status == certify.StatusSafe {
			...
			result.Reason = fmt.Sprintf(
				"selected first certified Harris candidate %s after %d encrypted trial(s)",
				candidate.ID,
				result.TrialsUsed,
			)
			return result, nil
		...
		if trial.FailureSignal == "" {
			...
			result.Reason = fmt.Sprintf(
				"no declared repair for Harris status=%s",
				trial.Status,
			)
			return result, nil
		}
	...
	result.Outcome = AdaptiveOutcomeNoSafe
	result.Reason = fmt.Sprintf(
		"no SAFE Harris candidate within trial budget %d",
		maxTrials,
	)
	return result, nil
}
