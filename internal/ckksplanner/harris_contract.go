...
const (
	BSDS500HarrisPatchExtractionV1 = "bsds500_harris_patch_extraction_v1"
	BSDS500HarrisModelSchemaV1     = "flipguard_bsds500_harris_holdout_v1"
	BSDS500HarrisModelType         = "harris_corner_response"
)

type HarrisContractOptions struct {
	ModelPath         string
...
type harrisModelArtifact struct {
...
type harrisValidationSample struct {
	ID            string
	Pixels        [5][5]float64
	PlainScore    float64
	PlainDecision bool
...
func DefaultPrimaryHarrisContractOptions() HarrisContractOptions {
	tabular := DefaultPrimaryTabularContractOptions()
	return HarrisContractOptions{
...
func BuildHarrisWorkloadContract(
	options HarrisContractOptions,
) (WorkloadContract, error) {
	normalizeHarrisContractOptions(&options)

	modelBytes, err := os.ReadFile(options.ModelPath)
	if err != nil {
		return WorkloadContract{}, fmt.Errorf(
			"read Harris model artifact: %w",
			err,
		)
	}
	model := harrisModelArtifact{}
	if err := json.Unmarshal(modelBytes, &model); err != nil {
		return WorkloadContract{}, fmt.Errorf(
			"parse Harris model artifact: %w",
			err,
		)
	}
	if err := validateHarrisModel(model); err != nil {
	...
	if err != nil {
		return WorkloadContract{}, fmt.Errorf(
			"read Harris validation data: %w",
			err,
		)
	}
	samples, archiveDigest, extractionDigest, err :=
		parseHarrisValidation(validationBytes, model)
	if err != nil {
		return WorkloadContract{}, err
	}
	...
	if sourceDigest != model.SourceArchive.SHA256 ||
		...
		return WorkloadContract{}, fmt.Errorf(
			"Harris source archive digest mismatch: source=%s model=%s validation=%s",
			sourceDigest,
	...
	if extractionDigest != model.ExtractionPolicyDigest {
		return WorkloadContract{}, fmt.Errorf(
			"Harris extraction policy digest mismatch",
		)
	}

	graph := benchmarks.NewHarrisCornerRescaleGraph()
	if err := graph.Validate(); err != nil {
		return WorkloadContract{}, fmt.Errorf(
			"validate Harris planner graph: %w",
			err,
		)
	}
	...
	for index, sample := range samples {
		inputs := harrisInputs(sample.Pixels)
		...
		if err != nil {
			return WorkloadContract{}, fmt.Errorf(
				"evaluate Harris validation row %d: %w",
				index+2,
				err,
			)
		}
		if !closeFloatAtTolerance(
			...
			return WorkloadContract{}, fmt.Errorf(
				"Harris validation row %d score %.12g does not match graph %.12g",
				index+2,
				sample.PlainScore,
				plain.Output,
			)
		...
		if decision != sample.PlainDecision {
			return WorkloadContract{}, fmt.Errorf(
				"Harris validation row %d plaintext decision mismatch",
				index+2,
			)
		}
		margin := math.Abs(
	...
	if vCert == 0 || math.IsInf(protectedMargin, 1) {
		return WorkloadContract{}, fmt.Errorf(
			"Harris validation has no certifiable sample above margin floor %.12g",
			options.MarginFloor,
		)
	}
	...
	if err != nil {
		return WorkloadContract{}, fmt.Errorf(
			"analyze Harris graph sensitivity: %w",
			err,
		)
	}
	...
	if aggregateSensitivity <= 0 {
		return WorkloadContract{}, fmt.Errorf(
			"derived Harris aggregate sensitivity is not positive",
		)
	}
	...
	graphSummary.Notes = []string{
		"derived from the exact Harris rescale-aware plaintext DAG",
		"products use the algebraic square-only identity 2ab=(a+b)^2-a^2-b^2",
	...
	contract := WorkloadContract{
		...
		InputMaterialization: &InputMaterializationContract{
			...
			PreprocessingMethod:  BSDS500HarrisPatchExtractionV1,
	...
	if err := contract.Validate(); err != nil {
		return WorkloadContract{}, fmt.Errorf(
			"validate derived Harris workload contract: %w",
			err,
		)
	}
	return contract, nil
...
func normalizeHarrisContractOptions(options *HarrisContractOptions) {
	defaults := DefaultPrimaryHarrisContractOptions()
...
func validateHarrisModel(model harrisModelArtifact) error {
	if model.SchemaVersion != BSDS500HarrisModelSchemaV1 {
		return fmt.Errorf(
			"unsupported Harris model schema %q",
			model.SchemaVersion,
		)
	}
	if model.DatasetID != "bsds500" ||
		model.ModelID != BSDS500HarrisModelType ||
		model.ModelType != BSDS500HarrisModelType ||
		model.InputDim != 25 ||
		model.GraphFormula != "det(M)-0.04*trace(M)^2" {
		return fmt.Errorf("Harris model identity or graph formula changed")
	}
	if !finite(model.DecisionThreshold) ||
		model.DecisionThreshold <= 0 {
		return fmt.Errorf("Harris decision threshold must be positive")
	}
	if model.PackingScope != ScalarReplicatedPackingV1 {
		return fmt.Errorf("unsupported Harris packing scope")
	}
	if model.ExtractionPolicyID != BSDS500HarrisPatchExtractionV1 ||
		strings.TrimSpace(model.ExtractionPolicyDigest) == "" {
		return fmt.Errorf("Harris extraction policy binding changed")
	}
	...
	if model.DirectPolicyReference.PolicyID != direct.PolicyID ||
		...
		return fmt.Errorf("Harris direct policy reference changed")
	}
	if !strings.Contains(
		...
		return fmt.Errorf(
			"Harris model must remain an explicit graph-adapter extension",
		)
	}
	if err := validateSHA256(model.SourceArchive.SHA256); err != nil {
		return fmt.Errorf("Harris source archive digest: %w", err)
	}
	if err := validateSHA256(model.ExtractionPolicyDigest); err != nil {
		return fmt.Errorf("Harris extraction policy digest: %w", err)
	}
	return nil
}

func parseHarrisValidation(
	data []byte,
	model harrisModelArtifact,
) (
	[]harrisValidationSample,
	string,
	string,
	error,
) {
	...
	if err != nil {
		return nil, "", "", fmt.Errorf(
			"read Harris validation CSV: %w",
			err,
		)
	}
	if len(records) < 2 {
		return nil, "", "", fmt.Errorf(
			"Harris validation CSV has no data rows",
		)
	}
	...
	for row := 0; row < 5; row++ {
		for column := 0; column < 5; column++ {
			required = append(required, fmt.Sprintf("p%d%d", row, column))
		}
	}
	for _, field := range required {
		if _, ok := header[field]; !ok {
			return nil, "", "", fmt.Errorf(
				"Harris validation CSV missing %s",
				field,
			)
		}
	}

	samples := make([]harrisValidationSample, 0, len(records)-1)
	...
	for rowIndex, record := range records[1:] {
		field := func(name string) (string, error) {
			...
			if index >= len(record) {
				return "", fmt.Errorf(
					"Harris validation row %d missing %s",
					rowIndex+2,
					name,
				)
			}
		...
		if id == "" {
			return nil, "", "", fmt.Errorf(
				"Harris validation row %d has empty row_id",
				rowIndex+2,
			)
		}
		if _, exists := seen[id]; exists {
			return nil, "", "", fmt.Errorf(
				"duplicate Harris row_id %s",
				id,
			)
		}
		seen[id] = struct{}{}
		...
		for row := 0; row < 5; row++ {
			for column := 0; column < 5; column++ {
				name := fmt.Sprintf("p%d%d", row, column)
				raw, _ := field(name)
				...
				if err != nil ||
					...
					value > 1 {
					return nil, "", "", fmt.Errorf(
						"Harris validation row %d invalid %s",
						rowIndex+2,
						name,
					)
				}
		...
		if err != nil || !finite(score) {
			return nil, "", "", fmt.Errorf(
				"Harris validation row %d invalid plaintext_score",
				rowIndex+2,
			)
		}
		...
		if !closeFloatAtTolerance(score, recomputed, 1e-12) {
			return nil, "", "", fmt.Errorf(
				"Harris validation row %d plaintext score changed",
				rowIndex+2,
			)
		}
		...
		if err != nil ||
			...
			return nil, "", "", fmt.Errorf(
				"Harris validation row %d threshold changed",
				rowIndex+2,
			)
		}
		...
		if err != nil || decision != (score >= threshold) {
			return nil, "", "", fmt.Errorf(
				"Harris validation row %d decision changed",
				rowIndex+2,
			)
		}
		...
		if rowIndex == 0 {
			...
			return nil, "", "", fmt.Errorf(
				"Harris validation provenance changes at row %d",
				rowIndex+2,
			)
		}
		samples = append(samples, harrisValidationSample{
			ID:            id,
			Pixels:        pixels,
			PlainScore:    score,
...
func harrisInputs(pixels [5][5]float64) map[ir.NodeID]float64 {
