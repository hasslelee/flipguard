#!/usr/bin/env python3
"""Export a deterministic BSDS500 Sobel decision-integrity holdout."""
...
SCHEMA_VERSION = "flipguard_bsds500_sobel_holdout_v1"
EXTRACTION_POLICY_ID = "bsds500_sobel_patch_extraction_v1"
MODEL_ID = "sobel_edge_score"
MODEL_TYPE = "sobel_edge_score"
ARCHIVE_URL = (
...
def patch_centers(
    ...
    if width < 3 or height < 3:
        raise ValueError(f"image {image_id} is smaller than 3x3")
    available = (width - 2) * (height - 2)
    if available < count:
        raise ValueError(
    ...
    while len(centers) < count:
        ...
        x = 1 + int.from_bytes(digest[:8], "big") % (width - 2)
        y = 1 + int.from_bytes(digest[8:16], "big") % (height - 2)
        center = (x, y)
...
def sobel_score(values: list[float]) -> float:
    (
        p00,
        p01,
        p02,
    ...
    for path in paths:
        ...
        with Image.open(path) as source:
            ...
            for patch_index, (center_x, center_y) in enumerate(centers):
                ...
                row["plaintext_score"] = sobel_score(values)
                rows.append(row)
                row_id += 1
    return rows
...
def write_rows(
    ...
    with path.open("w", newline="", encoding="ascii") as handle:
        ...
        for row in rows:
            ...
            output["plaintext_score"] = format_float(score)
...
def build(
    ...
    manifest = {
        ...
        "claim_scope": {
            "operator": "single-patch Sobel gradient-energy threshold",
            "statistical_unit": "image_cluster",
            "not_claimed": [
                "full_image_edge_detection_accuracy",
...
def verify(
    ...
    with tempfile.TemporaryDirectory(
        prefix="flipguard-bsds500-sobel-"
    ) as temporary:
