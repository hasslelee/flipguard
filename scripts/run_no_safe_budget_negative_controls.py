...
SOURCE_PACKAGE_DIRS = (
    Path("internal/certify"),
    Path("internal/ckksbackend"),
    Path("internal/ckksplanner"),
    Path("internal/tuner"),
)
...
def runtime_source_files() -> tuple[Path, ...]:
    package_files = (
        ...
        for package_dir in SOURCE_PACKAGE_DIRS
        for path in (REPO_ROOT / package_dir).glob("*.go")
...
def execute_workload(
    ...
    write_json(
        binding_path,
        {
            ...
            "result": {
                "path": portable_path(result_path),
