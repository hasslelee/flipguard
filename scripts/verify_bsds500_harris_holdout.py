#!/usr/bin/env python3
"""Fail-closed verifier for the derived BSDS500 Harris holdout."""
...
EXPORTER_PATH = REPO_ROOT / "scripts/export_bsds500_harris_holdout.py"
...
def verify_partition(
    ...
    if len(rows) != 200:
        raise ValueError(
            f"{expected_partition} has {len(rows)} rows, expected 200"
        )
    row_ids: set[int] = set()
    ...
    for index, row in enumerate(rows):
        ...
        components = EXPORTER.harris_components(pixels)
        for name, expected_value in components.items():
            if not math.isclose(
                ...
                raise ValueError(f"{name} changed in row {row_id}")
...
def verify(
    ...
    if model["schema_version"] != EXPORTER.SCHEMA_VERSION or \
        ...
        raise ValueError("Harris artifact schema changed")
    if model["model_type"] != EXPORTER.MODEL_TYPE or \
            model["graph_formula"] != "det(M)-0.04*trace(M)^2":
        raise ValueError("Harris graph identity changed")
    ...
    if model["extraction_policy_digest"] != extraction_digest or \
        ...
        raise ValueError("Harris extraction policy digest changed")
    ...
    if model["source_archive"]["sha256"] != archive_digest or \
        ...
        raise ValueError("Harris source archive binding changed")
