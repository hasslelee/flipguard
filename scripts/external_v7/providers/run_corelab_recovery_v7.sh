...
prefix_passed() {
  local prefix="$1"
  local state
  for state in "$STATUS"/"$prefix"*/current_stage.txt; do
    [[ -f "$state" ]] && [[ "$(<"$state")" == PASS ]] && return 0
  done
  return 1
}

test "$(git -C external/v7/sources/elasm rev-parse HEAD)" = 3c37c11b29ca480525bb6681e0254bdf90029425
test "$(git -C external/v7/sources/seal-4.0.0 describe --tags --exact-match)" = v4.0.0
test -x external/v7/builds/elasm/compiler/bin/hecate-opt
test -f external/v7/builds/elasm/compiler/lib/libSEAL_HEVM.so
...
if ! stage_passed 0007-hecate-deps; then
...
if ! prefix_passed 0008-hecate-build; then
  if [[ -e external/v7/builds/hecate ]]; then
    mkdir -p external/v7/failed_intermediates/corelab
    mv external/v7/builds/hecate external/v7/failed_intermediates/corelab/hecate-build-attempt1
  fi
...
if ! stage_passed 0009-hecate-runtime-image; then
...
if ! stage_passed 0010-hecate-runtime; then
...
if ! prefix_passed 0011-hecate-e2e; then
