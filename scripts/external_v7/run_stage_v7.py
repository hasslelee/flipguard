#!/usr/bin/env python3
"""Run one V7 provider stage with durable logs, heartbeat, and host locking."""
...
def main() -> int:
    args = parse_args()
    status_root = ROOT / "external/v7/status" / args.provider
    run_status = status_root / args.run_id
    log_root = ROOT / "external/v7/logs" / args.provider / args.run_id
    if run_status.exists() or log_root.exists():
        raise ValueError(f"refusing to overwrite V7 run {args.provider}/{args.run_id}")
    ...
    environment_record = {
        "schema_version": "flipguard_external_v7_stage_environment_v1",
    ...
    for relative, value in {
        ...
        "service_unit.txt": os.environ.get("FLIPGUARD_V7_SERVICE_UNIT", unit) + "\n",
    ...
    lock_path = Path("/tmp/flipguard-v7-measurement.lock")
    ...
    try:
        ...
        raise RuntimeError("another V7 build or measurement holds the host lock") from error
    ...
    with process_csv.open("x", newline="", encoding="utf-8") as proc_handle, resource_csv.open(
        ...
        monitor_thread = threading.Thread(target=monitor, name="v7-heartbeat", daemon=True)
    ...
    manifest = {
        "schema_version": "flipguard_external_v7_stage_run_v1",
        "provider": args.provider,
        "run_id": args.run_id,
        "stage": args.stage,
...
if __name__ == "__main__":
    try:
        ...
        print(f"V7 stage runner error: {error}", file=sys.stderr)
        raise SystemExit(2)
