...
def main() -> int:
    ...
    if not 15 <= args.waterline <= 50 or args.seed != 20260805:
        raise ValueError("ELASM V6 frozen plan or seed changed")
    root = args.runtime_root.resolve()
    context = args.context_root.resolve()
    if context.exists() or args.output_json.exists() or args.output_npz.exists():
        raise FileExistsError("refusing to overwrite ELASM V6 plan material")
    ...
    record = {
        "schema_version": "flipguard_external_v6_elasm_plan_result_v1",
        "mode": args.mode,
