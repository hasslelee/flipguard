...
restore_full_outputs() {
  ...
  if [[ -d "${FULL_CACHE_DIR}/ckks_harris_corner_tuner" ]]; then
    rm -rf "${HARRIS_RESULT_DIR}"
    cp -a "${FULL_CACHE_DIR}/ckks_harris_corner_tuner" "${HARRIS_RESULT_DIR}"
  fi
}
...
echo
echo "Running planner-guided Harris Corner execution..."
go run ./cmd/flipguard \
  -experiment ckks_harris_corner_tuner \
  -ckks-profile-names "${HARRIS_PROFILES}" \
  -ckks-timing-measurement-runs 3
