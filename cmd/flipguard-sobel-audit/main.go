...
func main() {
	if err := run(os.Args[1:], os.Stdout); errors.Is(err, flag.ErrHelp) {
		...
		fmt.Fprintf(os.Stderr, "flipguard-sobel-audit: %v\n", err)
		os.Exit(1)
	}
}

func run(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet("flipguard-sobel-audit", flag.ContinueOnError)
	flags.SetOutput(stdout)
	selectionPath := flags.String(
		"selection",
		"",
		"frozen Sobel selection JSON",
	)
	auditPath := flags.String(
		"audit",
		"",
		"locked-audit Sobel patch CSV",
	)
	...
	manifestPath := flags.String(
		"extraction-manifest",
		"",
		"frozen Sobel extraction manifest",
	)
	...
	result, err := ckksplanner.RunLockedSobelAudit(
		ckksplanner.SobelLockedAuditOptions{
	...
	if err != nil {
		return fmt.Errorf("marshal Sobel audit: %w", err)
	}
	encoded = append(encoded, '\n')
	if err := os.WriteFile(*outputPath, encoded, 0o644); err != nil {
		return fmt.Errorf("write Sobel audit: %w", err)
	}
	fmt.Fprintf(stdout, "sobel_audit=%s outcome=%s\n", *outputPath, result.Outcome)
	return nil
}
