...
func main() {
	if err := run(os.Args[1:], os.Stdout); errors.Is(err, flag.ErrHelp) {
		...
		fmt.Fprintf(os.Stderr, "flipguard-harris-autotune: %v\n", err)
		os.Exit(1)
	}
}
...
func run(args []string, stdout io.Writer) error {
	flags := flag.NewFlagSet(
		"flipguard-harris-autotune",
		flag.ContinueOnError,
	)
	flags.SetOutput(stdout)
	modelPath := flags.String("model", "", "frozen Harris model artifact")
	validationPath := flags.String(
		"validation",
		"",
		"configuration-validation Harris patch CSV",
	)
	...
	options := ckksplanner.DefaultPrimaryHarrisContractOptions()
	options.ModelPath = *modelPath
	options.ValidationPath = *validationPath
	options.SourceArchivePath = *sourceArchivePath
	options.SplitID = *splitID
	contract, err := ckksplanner.BuildHarrisWorkloadContract(options)
	if err != nil {
		return err
	}
	...
	var value any = plan
	if !*planOnly {
		result, err := ckksplanner.RunAdaptiveHarrisAutotune(plan)
		if err != nil {
			return err
		}
		value = result
	...
	if err != nil {
		return fmt.Errorf("marshal Harris result: %w", err)
	}
	...
	if err := os.WriteFile(*outputPath, encoded, 0o644); err != nil {
		return fmt.Errorf("write Harris result: %w", err)
	}
	fmt.Fprintf(stdout, "harris_result=%s\n", *outputPath)
	return nil
}
